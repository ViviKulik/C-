...
void GUI::initializeGuiComponents() {
    ...
    tableMovies->setHorizontalHeaderLabels(QStringList() << "Title" << "Genre" << "Year" << "Main actor");
    ...
    listMovies->setObjectName("Movies List");
...
void GUI::connectSignalsSlots() {
    ...
    QObject::connect(btnAddMovie, &QPushButton::clicked, this, &GUI::GuiAddMovieWindow);
    QObject::connect(btnModifyMovie, &QPushButton::clicked, this, &GUI::GuiModifyMovieWindow);
    QObject::connect(btnFindMovie, &QPushButton::clicked, this, &GUI::GuiFindMovieWindow);
    QObject::connect(btnDeleteMovie, &QPushButton::clicked, this, &GUI::GuiDeleteMovie);
    ...
    //WISHLIST
    QObject::connect(btnAddWishlist, &QPushButton::clicked, this, &GUI::GuiAddWishlist);
    QObject::connect(btnAddRandomWishlist, &QPushButton::clicked, this, &GUI::GuiAddRandomWishlistWindow);
    ...
    QObject::connect(btnExportWishlist, &QPushButton::clicked, this, &GUI::GuiExportWishlistWindow);
...
void GUI::reloadMoviesTabel(QTableWidget* tbl, const vector<Movie>& movies) {
    tbl->clearContents();
...
void GUI::reloadMoviesList(QListWidget* list, const vector<Movie>& movies) {
    list->clear();
...
void GUI::GuiAddMovieWindow() {
    ...
    QObject::connect(btnAdd, &QPushButton::clicked, this, &GUI::GuiAddMovie);
    lyAddWindow->addWidget(btnAdd);
    AddWindow->show();
}

void GUI::GuiAddMovie() {
...
void GUI::GuiModifyMovieWindow() {
    ...
    QObject::connect(btnModify, &QPushButton::clicked, this, &GUI::GuiModifyMovie);
    lyModifyWindow->addWidget(btnModify);
    ModifyWindow->show();
}


void GUI::GuiModifyMovie() {
    string title = editTitle->text().toStdString();
    string genre = editGenre->text().toStdString();
    int year = editYear->text().toInt();
    string actor = editActor->text().toStdString();
...
void GUI::GuiFindMovieWindow() {
    ...
    QObject::connect(btnFind, &QPushButton::clicked, this, &GUI::GuiFindMovie);
    lyFindWindow->addWidget(btnFind);
    FindWindow->show();
}


void GUI::GuiFindMovie() {
    string title = editTitle->text().toStdString();
    int year = editYear->text().toInt();
...
void GUI::GuiDeleteMovie() {
    QList selectedMovies = tableMovies->selectedItems();
    if (selectedMovies.empty()) {
        QMessageBox::warning(this, "Warning", QString::fromStdString("Select the movies you want to delete from the "
                                                                     "list"));
    }
    else {
        while (!selectedMovies.empty()) {
            string title = selectedMovies[0]->text().toStdString();
            int year = selectedMovies[2]->text().toInt();
            try {
                service.sterge(title, year);
                QMessageBox::information(this, "Info",
                                         QString::fromStdString("The movie has been deleted successfully"));
                reloadMoviesTabel(tableMovies, service.getAllMovies());
                reloadMoviesList(listMovies, service.getAllMovies());
                ReloadDynamicButtons();
            }
...
void GUI::GuiAddRandomWishlistWindow() {
...
void GUI::GuiExportWishlistWindow() {
